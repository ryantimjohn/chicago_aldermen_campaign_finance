...
def getStats(inList,cl): #compute metrics for a list of donations. Cl is the bayesian classifier object
    ...
    return [totalValue,median(values),float(individualMoney)/float(totalValue),float(smallMoney)/float(totalValue),float(individualN)/float(totalN), donorList, float(inChicagoValue)/float(totalValue), float(inWardValue)/float(totalValue),inWardN,]
...
for personID in range(0,len(aldernames)):
    factFile=open(aldernames[personID]+" report.txt",'w')
